---
name: medquery-pubmed
description: Search PubMed and return a synthesized, cited answer to a clinical or scientific question. Use whenever the user asks about medical evidence, treatment comparisons, drug effects, clinical outcomes, diagnostic accuracy, or anything requiring citations from the peer-reviewed biomedical literature. Also invoke when the user types /medquery-pubmed.
---

# medquery-pubmed — PubMed research
...
- The user explicitly invokes `/medquery-pubmed <question>`.
- The user asks for citations, references, or "what does the literature say".

Sibling skills in this plugin (e.g. `/medquery-trials`, `/medquery-fda`) may be better fits for registry data or regulatory questions as they're added — defer to those when the question is clearly about ongoing trials or drug safety signals rather than published literature.
...
If the user invoked `/medquery-pubmed` with no question, ask them what they want researched before launching the subagent.
